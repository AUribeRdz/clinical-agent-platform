...
// In demo mode accept the literal string "demo-token" as a valid bearer token
// so you can curl the API without generating a real JWT.
app.addHook('onRequest', async (request, reply) => {
  const open = ['GET /health', 'GET /mock-edc', 'POST /mock-edc']
  const key = `${request.method} ${request.routerPath ?? request.url}`
  if (open.some(r => key.startsWith(r))) return

  const header = request.headers.authorization ?? ''
  if (header === 'Bearer demo-token') return   // demo shortcut
...
// ── Mock EDC endpoint (used by the worker poller) ─────────────────────────────
...
// ── Subjects — submit a note, run the agent, apply HITL gate ─────────────────
...
app.post('/subjects', async (request, reply) => {
  ...
  // 2. Run the agent against the note
  const agentOutput = await runAgent(note, 'adverse_event')

  // 3. Apply HITL gate — check confidence against the field-type threshold
  ...
  // 4. Store the agent run (append-only)
  const { rows: [run] } = await pool.query(
    ...
    [
      record.id,
      agentOutput.prompt_version,
      note,
      JSON.stringify(agentOutput),
      agentOutput.confidence,
      status,
      agentOutput.field_type,
      'api',
    ]
  )
  ...
  return {
    ...
    output: agentOutput,
    hitl_required: status === 'requires_review',
    threshold_used: threshold,
  }
})

// ── HITL queue — list items awaiting review ───────────────────────────────────
...
// ── HITL decision — reviewer approves or rejects ──────────────────────────────
...
app.post('/hitl/:run_id/decide', async (request, reply) => {
  ...
  // Verify the run exists and is awaiting review
  const { rows: [run] } = await pool.query(
    'SELECT id, status FROM agent_runs WHERE id = $1',
    [run_id]
  )
  ...
  // Record the decision (append-only — we INSERT a decision row, not UPDATE agent_runs)
  ...
  // Update agent_run status (this is the one permitted UPDATE — the decision itself is immutable)
